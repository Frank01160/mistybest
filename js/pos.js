  ...
  list.forEach((product) => {
    ...
    const priceLabel =
      product.unitType === "single"
        ...
        : `${formatKsh(product.pricePerMinor)} / ${product.minorUnitName}`;
...
function openUnitModal(product) {
  ...
  if (product.unitType === "single") {
    ...
    const fractionWrap = document.getElementById("fractionPresets");
    fractionWrap.innerHTML = "";
    FRACTION_PRESETS.forEach((preset) => {
      const chip = document.createElement("button");
      chip.type = "button";
      chip.className = "fraction-chip";
      chip.textContent = preset.label;
      chip.addEventListener("click", () => {
        fractionWrap.querySelectorAll(".fraction-chip").forEach((c) => c.classList.remove("active"));
        chip.classList.add("active");
        document.getElementById("majorQtyInput").value = preset.value;
        modalQty = preset.value;
        validateModalQty();
      });
      fractionWrap.appendChild(chip);
    });
    const majorInput = document.getElementById("majorQtyInput");
    majorInput.value = "";
    majorInput.oninput = () => {
      fractionWrap.querySelectorAll(".fraction-chip").forEach((c) => c.classList.remove("active"));
      modalQty = parseFloat(majorInput.value) || 0;
      validateModalQty();
    };
...
document.getElementById("unitModalAdd").addEventListener("click", () => {
  ...
  if (modalUnitMode === "single") {
    ...
    pricePerUnitSold = product.pricePerMinor * product.minorPerMajor;
    qty = modalQty;
  } else {
