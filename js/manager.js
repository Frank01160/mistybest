...
function renderProductsTable() {
  ...
  tbody.innerHTML = list
    .map((p) => {
      ...
      const priceLabel = p.unitType === "single" ? `${formatKsh(p.price)}/${p.unitName}` : `${formatKsh(p.pricePerMinor)}/${p.minorUnitName}`;
...
function wireProductModal() {
  ...
  // Live-computed hints for double-unit pricing/stock
  ...
  [minorPerMajorInput, pricePerMinorInput].forEach((el) =>
    el.addEventListener("input", () => {
      const minorPerMajor = parseFloat(minorPerMajorInput.value) || 0;
      const pricePerMinor = parseFloat(pricePerMinorInput.value) || 0;
      document.getElementById("majorPriceHint").textContent = minorPerMajor && pricePerMinor
        ? `Major unit price ≈ ${formatKsh(pricePerMinor * minorPerMajor)}`
        : "Major unit price will be calculated automatically.";
    })
  );
...
function openProductModal(product) {
  ...
  document.getElementById("majorPriceHint").textContent =
    product?.minorPerMajor && product?.pricePerMinor
      ? `Major unit price ≈ ${formatKsh(product.pricePerMinor * product.minorPerMajor)}`
      : "Major unit price will be calculated automatically.";
...
async function saveProduct(e) {
  ...
  try {
    ...
    if (productModalUnitType === "single") {
      ...
      payload = { ...payload, unitType: "double", majorUnitName, minorUnitName, minorPerMajor, pricePerMinor, stockMinorUnits, unitName: null, price: null };
    }
